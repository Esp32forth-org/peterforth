...
#define USER_WORDS \  
    Y(ALPHA, voice.say(  sp2_ALPHA ); NIP ) \                            
     ...
     Y(PUMPS, voice.say(  sp4_PUMPS ); NIP ) \  
      Y(WHATISBID, voice.say(  spWHAT_IS_THY_BIDDING ); NIP ) \           
     Y(HASTALAVISTA, voice.say(  spHASTA_LA_VISTA ); NIP ) \           
     Y(SMALLSTEP, voice.say(  spONE_SMALL_STEP ); NIP ) \  
      Y(REMEMBER, voice.say( spt_REMEMBER ); NIP ) \ 
     Y(MORE, voice.say( spt_MORE ); NIP ) \ 
      Y(NAME_, voice.say( spt_NAME ); NIP ) \ 
       Y(MUST, voice.say( spt_MUST ); NIP ) \ 
       Y(LEARN, voice.say( spt_LEARN  ); NIP ) \ 
        Y(THATSRIGHT, voice.say( spt_THAT_IS_RIGHT ); NIP ) \ 
         Y(THINGS, voice.say( spt_THINGS ); NIP ) \ 
          Y(TRYAGAIN_, voice.say( spt_TRY_AGAIN ); NIP ) \ 
         Y(UNDERSTAND, voice.say( spt_UNDERSTAND); NIP ) \             
         Y(WORDS_, voice.say( spt_WORDS ); NIP ) \ 
        Y(WILL, voice.say( spt_WILL ); NIP ) \ 
       Y(WHO, voice.say( spt_WHO); NIP ) \ 
      Y(SORRY, voice.say(  spt_SORRY); NIP ) \    
       Y(PROBLEM, voice.say( spt_PROBLEM   ); NIP ) \ 
       Y(PLEASE, voice.say( spt_PLEASE); NIP ) \ 
      Y(OH, voice.say(  spt_OH); NIP ) \      
       Y(YOU, voice.say(  spt_YOU ); NIP ) \   
       Y(THE_, voice.say(  spt_THE ); NIP ) \   
      Y(GOOD, voice.say( spt_GOOD  ); NIP ) \  
      Y(GOODBYE, voice.say(  spt_GOODBYE ); NIP ) \    
     Y(GUESS, voice.say(  spt_GUESS  ); NIP ) \  
     Y(HAND, voice.say(   spt_HAND  ); NIP ) \   
     Y(GO, voice.say(   spt_GO  ); NIP ) \   
     Y(FROM, voice.say(   spt_FROM  ); NIP ) \   
     Y(FOURTH, voice.say(  spt_FOURTH ); NIP ) \   
     Y(FIND_, voice.say(   spt_FIND  ); NIP ) \    
     Y(NOT_, voice.say(  spt_NOT  ); NIP ) \  
     Y(NOW, voice.say(  spt_NOW  ); NIP ) \   
     Y(SAY, voice.say( spt_SAY  ); NIP ) \   
     Y(SAID, voice.say( spt_SAID  ); NIP ) \   
     Y(SO, voice.say( spt_SO ); NIP ) \   
     Y(TOGETHER, voice.say( spt_TOGETHER  ); NIP ) \   
      Y(LIKE, voice.say( spt_LIKE  ); NIP ) \   
      Y(HELLO, voice.say( spt_HELLO ); NIP ) \   
     Y(HELP, voice.say( spt_HELP  ); NIP ) \   
      Y(HERE_, voice.say( spt_HERE ); NIP ) \  
      Y(WAIT_, voice.say(  spt_WAIT ); NIP ) \  
      Y(MAKE, voice.say(  spt_MAKE ); NIP ) \  
